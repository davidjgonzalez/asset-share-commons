...
public class AssetRenditionStreamerImpl implements AssetRenditionStreamer {
    ...
    public AssetRenditionStream getAssetRendition(final SlingHttpServletRequest request,
        ...
        try {
            ...
            if (assetRenditionDownloadResponse.getStatusCode() > 302) {
                throw new AssetRenditionException(String.format("Response of [ %s ] from dispatched response is unacceptable. Unable to stream [ %s ].",
            ...
            throw new AssetRenditionException(String.format("Unable to fetch internal Asset Rendition output stream for [  %s @ %s ]", asset.getPath(), renditionName));
        } catch (IOException e) {
            throw new AssetRenditionException(String.format("Unable to fetch external Asset Rendition output stream for [  %s @ %s ]", asset.getPath(), renditionName));
        }
    }

    private AssetRenditionDownloadResponse sendDispatchForAssetRendition(final SlingHttpServletRequest realRequest,
    ...
    private boolean acceptedByAssetRenditionDispatcher(final AssetRenditionDispatcher assetRenditionDispatcher, final AssetRenditionParameters parameters) {
    ...
    private AssetRenditionStream fetchExternalRendition(String uri) throws IOException {
    ...
    @Activate
    protected void activate(final AssetRenditionStreamerImpl.Cfg cfg) {
        this.cfg = cfg;
    }
